...
/**
 * return_reported_page - Makes reported pages available for re-use.
 * @zone: zone from where the page was marked as MIGRATE_ISOLATE.
 * @phconf: object carrying the list of pages which were reported.
 */
static void return_reported_page(struct zone *zone,
...
static void scan_zone_bitmap(struct page_reporting_config *phconf,
			     ...
			     struct zone *zone)
{
	unsigned long setbit, watermark;
	struct page *page;
	int count = 0;

	watermark = low_wmark_pages(zone) +
		    (PAGE_REPORTING_MAX_PAGES << PAGE_REPORTING_MIN_ORDER);
	...
	for_each_set_bit(setbit, rbitmap->bitmap, rbitmap->nbits) {
		 ...
		 * is already reallocated or the zone has lower number of free
		 * pages.
		 */
		if (!PageBuddy(page) ||
		...
		if (count == phconf->max_pages) {
			...
			return_reported_page(zone, phconf);
...
static void page_reporting_wq(struct work_struct *work)
{
	...
	phconf = container_of(d_work, struct page_reporting_config,
			      reporting_work);
...
void __page_reporting_enqueue(struct page *page)
{
	...
	/*
	 * We should enqueue a job only if we have enough free pages in
	 * the zone and if this zone doesn't have an ongoing page reporting
	 * request.
	 */
	if (atomic_read(&rbitmap->free_pages) > phconf->max_pages &&
	    !atomic_cmpxchg(&phconf->state, PAGE_REPORTING_IDLE,
			    PAGE_REPORTING_ACTIVE))
		schedule_delayed_work(&phconf->reporting_work,
...
void page_reporting_disable(struct page_reporting_config *phconf)
{
	...
	cancel_delayed_work(&phconf->reporting_work);
...
int page_reporting_enable(struct page_reporting_config *phconf)
{
	...
	INIT_DELAYED_WORK(&phconf->reporting_work, &page_reporting_wq);
