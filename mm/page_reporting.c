			     ...
			     struct zone *zone)
{
	...
	/* Allow zone to initiate new reporting request */
	__clear_bit(ZONE_PAGE_REPORTING_REQUEST, &zone->flags);
}
...
void __page_reporting_enqueue(struct page *page)
{
	...
	if (atomic_read(&rbitmap->free_pages) < phconf->max_pages ||
	    test_bit(ZONE_PAGE_REPORTING_REQUEST, &zone->flags))
		goto out;

	__set_bit(ZONE_PAGE_REPORTING_REQUEST, &zone->flags);

	if (!atomic_fetch_inc(&phconf->refcnt))
